...
inline void b3DynamicTree::RayCast(T* callback, const b3RayCastInput& input) const
{
    ...
    /* Upstream recreates this stack each call, we keep it static
     * and just ensure it has no elements which has the effect that
     * this will grow to the max size it needs and never shrink */
    static b3Stack<uint32, 256> stack;
    stack.Clear();

    const b3Vec3& p1 = input.p1;
    const b3Vec3& p2 = input.p2;
    b3Vec3 r = p2 - p1;
    ...
    const b3Vec3& e1 = b3Vec3_x;
    const b3Vec3& e2 = b3Vec3_y;
    const b3Vec3& e3 = b3Vec3_z;
	...
	while (stack.IsEmpty() == false)
	{
        ...
        const b3Vec3 c = node->aabb.GetCenter();
        const b3Vec3 h = node->aabb.GetExtents();
        ...
        scalar s1 = b3Abs(sigma_1 + tau_1) - (b3Abs(sigma_1 - tau_1) + two * eta_1);
        if (s1 > 0)
		{
			continue;
		}
        ...
        tau_1 = t.y;
        eta_1 = h.y;
        s1 = b3Abs(sigma_1 + tau_1) - (b3Abs(sigma_1 - tau_1) + two * eta_1);
        ...
        s1 = b3Abs(b3Dot(v1, s)) - b3Dot(abs_v1, h);
        if (s1 > 0)
		{
			continue;
		}
