...
struct b3Vec3 
{
    ...
    void Normalize()
	{
        ...
        if (length_squared > B3_EPSILON)
		{
            scalar s = 1.0f / b3Sqrt(length_squared);
			x *= s;
			y *= s;
			z *= s;
        }
...
{
    scalar a = 1.0f / s;
    return b3Vec3(v.x * a, v.y * a, v.z * a);
}
...
{
    scalar length_squared = b3LengthSquared(v);
    if (length_squared > B3_EPSILON)
    {
        scalar s = 1.0f / b3Sqrt(length_squared);
        return b3Vec3(
